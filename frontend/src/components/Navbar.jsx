  ...
  return (
    <header className="navbar">
      ...
      <form className="nav-search" onSubmit={submitSearch}>
        ...
        <button type="submit" className="nav-search-btn" aria-label={t('Pesquisar')}><Icon name="search" size={18} /></button>
      </form>

      <div className="nav-actions">
        {isBuyer ? (
          <Link to="/comprador/cesta" className="nav-icon" aria-label={t('Cesta')}>
            <Icon name="cart" size={20} />
            {cartCount > 0 ? <span className="nav-badge">{cartCount}</span> : null}
          </Link>
        ) : null}
        <button className="nav-icon" onClick={onBell} aria-label={t('Notificações')}>
