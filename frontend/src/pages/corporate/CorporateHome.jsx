...
export default function CorporateHome() {
  ...
  return <main id="top" className="kixima-corp">
    ...
    <section className="section faq"><div className="section-heading"><p className="eyebrow">{t('PERGUNTAS FREQUENTES')}</p><h2>{t('O essencial sobre a')} <em>{t('KIXIMA.')}</em></h2></div><div className="faq-list">
      <details open><summary>{t('O que é a KIXIMA.NET?')}</summary><div className="faq-answer">{t('É um e-marketplace B2B que liga compradores a fornecedores e automatiza o ciclo de aprovisionamento, desde o catálogo e a ordem de compra até à fatura, receção e pagamento.')}</div></details>
      <details><summary>{t('A KIXIMA serve apenas o sector de Oil & Gas?')}</summary><div className="faq-answer">{t('Não. O Oil & Gas é o ponto de partida e a referência de exigência, mas a infraestrutura foi concebida para crescer para energia, indústria, logística, mineração, aviação e outros sectores.')}</div></details>
      <details><summary>{t('Como funciona o credenciamento único?')}</summary><div className="faq-answer">{t('A empresa submete dados e documentos para verificação. Uma vez validado, o registo é reconhecido pelas empresas compradoras que operam na plataforma, sem repetir a due diligence a cada nova relação.')}</div></details>
      <details><summary>{t('Como é tratada a fiscalidade angolana?')}</summary><div className="faq-answer">{t('A plataforma calcula IVA e retenção na fonte por linha da transação e preserva o valor fiscal histórico aplicado no momento da emissão da ordem de compra.')}</div></details>
      <details><summary>{t('Em quanto tempo o fornecedor é pago?')}</summary><div className="faq-answer">{t('O financeiro do comprador processa o pagamento até 7 dias após a aceitação da ordem de compra, com histórico rastreável de ponta a ponta.')}</div></details>
      <details><summary>{t('A utilização exige uma subscrição obrigatória?')}</summary><div className="faq-answer">{t('Depende do plano da empresa. O acesso base ao marketplace não exige subscrição obrigatória; funcionalidades avançadas (como contratos-quadro) fazem parte dos planos pagos.')}</div></details>
    </div></section>
