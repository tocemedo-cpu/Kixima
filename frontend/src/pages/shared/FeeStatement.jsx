    ...
    <div className="doc-page">
      ...
      <div className="pdoc-sheet">
        <header className="pdoc-head">
          ...
          <div className="pdoc-tagline">Plataforma de procurement com pagamento garantido</div>
        </header>
        ...
        <section className="pdoc-parties">
          <div className="pdoc-party">
            <div className="pdoc-lbl">EMITIDO POR / FROM</div>
            <strong>KIXIMA</strong>
            <div>Plataforma de procurement Oil &amp; Gas</div>
            <div>Luanda, Angola</div>
          </div>
          <div className="pdoc-party">
            <div className="pdoc-lbl">FORNECEDOR / SUPPLIER</div>
        ...
        <section className="pdoc-boxes">
          <div className="pdoc-box">
            <div className="pdoc-lbl">FÓRMULA DA TAXA</div>
            <strong>(nº de POs × {money(formula.perPo, cur)}) + {money(formula.perInvoice, cur)} por fatura</strong>
          </div>
          <div className="pdoc-box">
            <div className="pdoc-lbl">TOTAL POR COBRAR</div>
        ...
        <table className="pdoc-items">
          <thead>
            <tr>
              ...
              <th style={{ textAlign: 'right' }}>Taxa</th><th>Estado</th>
            </tr>
          </thead>
          <tbody>
            {fees.length === 0 ? (
              <tr><td colSpan={6} style={{ textAlign: 'center', color: '#888' }}>Ainda não há taxas geradas para esta empresa.</td></tr>
            ) : fees.map((f) => (
              <tr key={f.id}>
                <td>{d(f.createdAt)}</td>
                ...
                <td>{f.status === 'COBRADO' ? `Cobrada em ${d(f.chargedAt)}` : 'Pendente'}</td>
              </tr>
            ))}
          </tbody>
          <tfoot>
            <tr>
              <td colSpan={4} style={{ textAlign: 'right', fontWeight: 600 }}>Total gerado</td>
              <td style={{ textAlign: 'right', fontWeight: 600 }}>{money(kpis.totalAOA, cur)}</td>
              <td />
            </tr>
            <tr>
              <td colSpan={4} style={{ textAlign: 'right' }}>Já cobrado</td>
              <td style={{ textAlign: 'right' }}>{money(kpis.chargedAOA, cur)}</td>
              <td />
            </tr>
            <tr>
              <td colSpan={4} style={{ textAlign: 'right', fontWeight: 700 }}>POR COBRAR</td>
        ...
        <p className="pdoc-muted" style={{ marginTop: 18, fontSize: 11 }}>
          A Taxa KIXIMA é gerada automaticamente em cada pagamento processado na plataforma e é
          cobrada ao fornecedor, à parte da PO/Fatura. Este extrato serve como documento de
          cobrança; o pagamento é conciliado pela equipa KIXIMA. Dúvidas: suporte via Ajuda.
        </p>
      </div>
    </div>
  );
