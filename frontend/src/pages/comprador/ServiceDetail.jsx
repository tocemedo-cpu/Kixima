...
export default function ServiceDetail() {
  ...
  async function quote() {
    try { await api.post('/api/quotes', { supplierCompanyId: p.supplier?.id, items: [{ productId: p.id, quantity: 1 }] }); setMsg('Pedido de cotação enviado.'); }
    catch (e) { setError(e.message); }
  }
  async function submitReview(e) {
    e.preventDefault();
    try { await api.post(`/api/catalog/${p.id}/reviews`, { rating: Number(rating), comment: comment || undefined }); setMsg('Avaliação registada.'); setComment(''); loadReviews(p.id); const fresh = await api.get(`/api/catalog/slug/${slug}`); setP(fresh); }
    catch (e) { setError(e.message); }
  }

  return (
    <div>
      <button className="btn btn-ghost btn-sm" style={{ marginBottom: 14 }} onClick={() => navigate('/comprador/servicos')}>← Voltar aos serviços</button>
      ...
      <div className="grid-cols grid-2" style={{ alignItems: 'start' }}>
        <div className="card" style={{ overflow: 'hidden' }}>
          ...
          <div className="card-pad">
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 6 }}>
              <Stars value={p.rating || 0} /> <span style={{ fontSize: 13, color: 'var(--ink-600)' }}>{p.rating ? p.rating.toFixed(1) : '—'} {p.reviewCount ? `(${p.reviewCount} avaliações)` : ''}</span>
              {p.supplier?.verified ? <span className="svc-verified">Verificado</span> : null}
            </div>
            <p style={{ fontSize: 14, color: 'var(--ink-600)', marginTop: 12, lineHeight: 1.6 }}>{p.fullDescription || p.description || 'Sem descrição.'}</p>
            <div style={{ marginTop: 16, display: 'grid', gap: 8, fontSize: 13.5 }}>
              <Row label="Fornecedor">{p.supplier?.name}</Row>
              <Row label="Localização">{[p.city, p.province, p.country].filter(Boolean).join(', ') || '—'}</Row>
              {p.leadTimeDays ? <Row label="Prazo">{p.leadTimeDays} dias</Row> : null}
              {p.availability ? <Row label="Disponibilidade">{p.availability}</Row> : null}
              {p.certifications?.length ? <Row label="Certificações">{p.certifications.join(', ')}</Row> : null}
            </div>
          </div>
        </div>
        ...
        <div>
          <div className="card card-pad" style={{ marginBottom: 16 }}>
            ...
            <button className="btn btn-accent" style={{ width: '100%', marginTop: 16 }} onClick={quote}>Solicitar Cotação</button>
          </div>

          <div className="card card-pad">
            <strong style={{ fontSize: 13.5 }}>Avaliações</strong>
            <form onSubmit={submitReview} style={{ margin: '12px 0', display: 'flex', gap: 8, alignItems: 'flex-end', flexWrap: 'wrap' }}>
              ...
              <div className="field" style={{ margin: 0, flex: 1 }}><label>Comentário</label><input value={comment} onChange={(e) => setComment(e.target.value)} /></div>
              <button className="btn btn-ghost btn-sm" type="submit">Avaliar</button>
            </form>
            {reviews.length === 0 ? <p className="helptext" style={{ margin: 0 }}>Ainda sem avaliações.</p> : (
