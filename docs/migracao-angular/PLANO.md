...
| Explorar/Serviços/Detalhe/Comparar/Fornecedores — Comprador | `features/catalog/{services,explore,service-detail,supplier-compare}.component.ts`, `features/orders/suppliers.component.ts` | `pages/comprador/{Services,Explore,ServiceDetail,SupplierCompare,Suppliers}.jsx` | `GET /api/marketplace/{search,facets,compare}`, `POST/DELETE /api/marketplace/favorites[/:id]`, `POST /api/marketplace/saved-searches`, `GET/POST /api/catalog/:id/reviews`, `GET /api/buyer/suppliers?status=&q=` |
| Assinatura (plano/subscrição) — Company Admin + Financeiro | `features/company-admin/assinatura.component.ts`, `shared/components/subscription-banner.component.ts` | `pages/companyAdmin/Assinatura.jsx`, `components/SubscriptionBanner.jsx` | `GET /api/assinatura`, `GET /api/assinatura/canais`, `POST /api/assinatura/pedir`, `POST /api/assinatura/:id/{pagar-com,comprovativo,cancelar}` |
| Empresas/Contratos-quadro/Cobranças de subscrição — Admin do Sistema | `features/admin/{admin-companies,admin-contracts,cobrancas,addons.service}.ts` | `pages/adminSistema/{Companies,Contracts,Cobrancas}.jsx` | `GET /api/companies?type=&status=`, `GET/POST /api/contracts`, `GET /api/assinatura/fila`, `POST /api/assinatura/:id/confirmar`, `GET /api/addons/{catalogo,fila}`, `POST /api/addons/:id/{confirmar,cancelar}` |
| Perfil/Segurança/Notificações — partilhadas (todas as personas) | `features/account/{profile,security,two-factor-section,personal-data-section,notifications}.component.ts`, `shared/components/avatar-uploader.component.ts` | `pages/shared/{Profile,Security,Notifications}.jsx`, `components/AvatarUploader.jsx` | `GET /api/users/profile`, `POST/DELETE /api/users/me/avatar`, `PATCH /api/auth/password`, `GET/POST /api/auth/2fa/*`, `GET /api/users/me/dados-pessoais`, `POST /api/users/me/anonimizar`, `PATCH /api/notifications/:id/read` |
| Documentos imprimíveis (PO/Fatura/Extrato de Taxas) — partilhadas | `features/documents/{fee-statement,printable-document}.component.ts` | `pages/shared/{FeeStatement,PrintableDocument}.jsx` | `GET /api/purchase-orders/:id`, `GET /api/companies/:id/platform-fees` |
...
**Verificação ao vivo — Explorar/Serviços/Detalhe/Comparar/Fornecedores (Comprador)**: última página de prioridade 2 do Comprador. Todos os endpoints (`GET /api/marketplace/{search,facets,compare}`, `POST/DELETE /api/marketplace/favorites[/:id]`, `POST /api/marketplace/saved-searches`, `GET/POST /api/catalog/:id/reviews`, `GET /api/buyer/suppliers`) já estavam correctos e completos no Java — confirmados endpoint a endpoint por um agente de pesquisa dedicado antes de codificar, que revelou várias particularidades reais preservadas de propósito: `favorites`/`saved-searches`/GET `reviews` **não têm** `@RequireRole` no Java (qualquer utilizador autenticado, não só COMPRADOR — confirmado ao vivo com uma conta FORNECEDOR); `compare` e POST `reviews` **exigem** COMPRADOR (403 confirmado ao vivo para FORNECEDOR); `GET /api/buyer/suppliers` aceita COMPRADOR **ou** COMPANY_ADMIN (mais permissivo do que a rota Angular, que só é alcançável por COMPRADOR — sem impacto, o servidor continua a ser a fonte de verdade); POST `reviews` é um *upsert* que devolve um resumo (`{rating, reviewCount}`), nunca a avaliação criada — por isso o componente sempre recarrega a lista de avaliações e o produto depois de submeter; `compare` agrupa "o mesmo item" por nome normalizado OU código UNSPSC, e devolve entre 0 e 5 ofertas, sem mínimo garantido. Corrigido um desvio real do modelo `MarketplaceSearchParams`, que não tinha os filtros `country`/`certifications` que `Explore.jsx`/`Services.jsx` legitimamente enviam e que `MarketplaceService.java` legitimamente lê (confirmado por grep directo ao código Java, não só pelo relatório do agente). Todo o estado de filtros de `Explore.jsx` (não só `q`) é sincronizado no endereço via `ActivatedRoute`/`Router`, replicando a razão original em comentário no React: um link com filtros aplicados tem de poder ser partilhado ou recarregado sem perder o resultado. Testado ao vivo por HTTP directo com a conta Comprador de teste: adicionar/remover favorito (confirmado a reflectir-se em `isFavorite` na pesquisa seguinte), criar e apagar uma pesquisa guardada, GET/POST de avaliação (criada, confirmada na lista, apagada a seguir para não poluir a base de teste), comparação com uma oferta real, e o directório de fornecedores com KPIs reais. Fluxo completo também testado pela UI real com Playwright: as 5 páginas renderizam com dados reais e sem nenhum erro JS de página (a pesquisa de Explorar actualiza o endereço com `q=`/`category=` correctamente ao usar filtros). Um bug de compilação de template foi encontrado e corrigido nesta sessão: Angular não aceita arrow functions (`=>`) dentro de bindings (`NG5002`), ao contrário do JSX original que usava `.filter(v => v)` livremente — extraído para um helper `joinNonEmpty()` partilhado em `shared/domain.ts`, usado nos 4 componentes que precisavam de juntar cidade/país/certificações condicionalmente.

**Verificação ao vivo — Assinatura (plano/subscrição, Company Admin + Financeiro)**: contratos confirmados endpoint a endpoint por um agente de pesquisa dedicado antes de codificar (dado envolver dinheiro), que revelou várias particularidades reais capturadas no modelo: (1) `valorUsd` tem tipos DIFERENTES na mesma resposta — é **string** em `PlanoCobrancaDto` (`emAberto`/`historico`, BigDecimal serializado como string pelo customizer global do Jackson) mas é **número** em `Preco` (`opcoes[].preco`, anotado com um serializador próprio) — confirmado ao vivo (`"valorUsd":"5000"` em `emAberto` vs `"valorUsd":100` em `opcoes[0].preco`); (2) `POST /pedir`/`pagar-com`/`comprovativo`/`cancelar` devolvem sempre o `PlanoCobrancaDto` completo dessa cobrança (19 campos), nunca um "ack" nem o objecto `estado()` inteiro — por isso o componente sempre recarrega `GET /api/assinatura` a seguir, tal como o React; (3) o servidor **não** valida `telemovel` como obrigatório para EMIS/PayPay (só o formulário do React/Angular o exige, por UX) — preservado exactamente assim; (4) um gateway sem credenciais configuradas neste ambiente devolve **500 genérico** (`INTERNAL_ERROR`), não um erro de negócio estruturado — o componente trata este caso à parte com uma mensagem amigável a sugerir a transferência bancária, em vez de mostrar o erro técnico; (5) `POST /cancelar` só exige `COMPANY_ADMIN`/`ADMIN_SISTEMA` — **FINANCEIRO não pode cancelar**, confirmado com 403 ao vivo — mas isto já coincidia com o comportamento do React (o botão "Cancelar cobrança" só aparece para `podeEscolherPlano`, que já era `role === 'COMPANY_ADMIN'`), por isso não foi preciso nenhuma correcção de UI, só a confirmação de que a UI já estava certa por acidente. A rota `/empresa/assinatura` foi colocada FORA do bloco `empresa` (que só permite COMPANY_ADMIN) — é a única página do Company Admin também vista pelo Financeiro, com o seu próprio `roleGuard('COMPANY_ADMIN', 'FINANCEIRO')` — o `role.guard.ts` desta sessão já tinha um comentário a antecipar exactamente este caso. Testado ao vivo por HTTP directo com a conta Company Admin de teste: pedir um plano PRO (201, `valorUsd` string confirmado), pedir de novo com cobrança já aberta (409/CONFLICT), pagar com gateway num plano não elegível — PRO — (400/BUSINESS_RULE_VIOLATION), carregar comprovativo PDF válido (200, `status: COMPROVATIVO_ENVIADO`), cancelar a cobrança mesmo já com comprovativo enviado (200 — só `CONFIRMADA`/`CANCELADA` bloqueiam o cancelamento, `COMPROVATIVO_ENVIADO` continua cancelável, confirmado ao vivo), e RBAC nos dois sentidos (Financeiro: 200 em GET estado/canais, 403 em pedir e em cancelar). Cobrança de teste apagada da base a seguir. Fluxo completo também testado pela UI real com Playwright nas duas personas: a página carrega com dados reais e sem nenhum erro JS, o botão "Cancelar cobrança" não aparece para o Financeiro, e o Financeiro é correctamente redireccionado ao tentar `/empresa` (guardado só para COMPANY_ADMIN). Nota de âmbito: `SubscriptionBannerComponent` foi portado e é usado dentro da própria página de Assinatura (`mostrarBotao=false`), mas o aviso GLOBAL em qualquer página (`AppLayout.jsx`) fica fora desta migração — depende da barra lateral por persona ainda não portada para o `ShellComponent` (ver `shell.component.ts`, deliberadamente minimal nesta fase).

**Verificação ao vivo — Empresas/Contratos-quadro/Cobranças de subscrição (Admin do Sistema)**: `Companies.jsx` reutiliza por completo `CompaniesService`/`CompanyListItem`/`CompanyDetail` já existentes (de `DueDiligence.jsx`), só ganhando o filtro `type` (confirmado ao vivo como um filtro AND independente e genuíno de `status` no Java, `findByStatusAndTypeOrderByCreatedAtDesc`). `Contracts.jsx` e `Cobrancas.jsx` exigiram um agente de pesquisa dedicado antes de codificar (dinheiro real), que revelou várias particularidades: (1) `POST /api/contracts` exige que `validFrom`/`validUntil` sejam um **instante ISO-8601 completo**, não a data "YYYY-MM-DD" que o `<input type=date>` produz — o desserializador de `Instant` do Jackson rejeita isso com 422; o componente converte com `new Date(v).toISOString()` antes de enviar; (2) o servidor **não valida** o tipo/estado das empresas escolhidas nem que `validUntil` seja posterior a `validFrom` — confirmado como lacunas reais do Java, não replicadas no cliente por não haver nada para replicar; (3) `GET /api/assinatura/fila` (lado KIXIMA) tem uma forma bem mais rica do que a suposição inicial — `emAberto[]` inclui `company:{id,name,plan}` (o `estado()` do lado da empresa omite-o), mais um campo `vencidas` (o conjunto completo antes de se dividir em `emGrace`/`restritas`); (4) o domínio `/api/addons` (Automatic PO Robot) foi modelado de raiz — `AddonCobrancaDto` é uma estrutura própria no Java, semelhante mas distinta de `PlanoCobrancaDto`, com a mesma particularidade `valorUsd`-como-string; `GET /api/addons/catalogo` não tem restrição de papel nenhuma (confirmado ao vivo, acessível por COMPANY_ADMIN); (5) `POST /api/assinatura/:id/confirmar` só aceita cobranças com `comprovativoUrl` preenchido (senão 400) e `notas` é inteiramente opcional. Testado ao vivo por HTTP directo: pedido de plano PRO → comprovativo → confirmação pelo Admin Sistema (plano da empresa mudou de facto para PRO, `validoAte` a 1 ano, revertido a seguir), `POST /api/contracts` recusado com plano CORE (400/PLANO_INSUFICIENTE) e aceite ao subir temporariamente o plano do cliente para PRO (201, `totalValue`/`usedValue` strings, `clientCompany`/`supplierCompany` omitidos ao contrário da listagem — revertido a seguir), RBAC confirmado nos dois sentidos (`GET /api/addons/fila` 403 para COMPANY_ADMIN; `POST /api/contracts` 403 para COMPANY_ADMIN a criar em nome de outra empresa). Fluxo completo também testado pela UI real com Playwright nas 3 páginas, sem nenhum erro JS.

**Verificação ao vivo — Perfil/Segurança/Notificações (partilhadas por todas as personas)**: contratos confirmados endpoint a endpoint por um agente de pesquisa dedicado antes de codificar (senha, 2FA e eliminação de dados pessoais são fluxos sensíveis), que revelou várias particularidades reais: (1) `GET /api/users/profile` devolve um `kind` (`"admin"|"supplier"|"buyer"`) que decide o conjunto de `cards` — não há um `money:false` explícito, o campo é omitido nas cartas que não são de dinheiro (`@JsonInclude(NON_NULL)`), e `value` é sempre um número cru mesmo nas cartas de dinheiro (a formatação com `formatMoney()` continua inteiramente do lado do cliente); (2) `POST`/`DELETE /api/users/me/avatar` devolvem sempre o utilizador público completo, nunca só `{avatarUrl}`; (3) a política de senha do servidor é mais rigorosa do que os 10 caracteres do formulário — exige 12 para papéis sensíveis (`COMPANY_ADMIN`/`FINANCEIRO`/`ADMIN_SISTEMA`) e recusa uma lista de senhas comuns/da marca — **confirmado ao vivo**: `PATCH /api/auth/password` recusou repor a senha de demonstração original (`Kixima@123`) com `422 VALIDATION_ERROR` ("Esta senha é das mais usadas no mundo…"), obrigando a restaurar o hash directamente na base de teste a partir doutra conta com a mesma senha; (4) mudar a senha sobe o `tokenVersion` e invalida a PRÓPRIA sessão actual, não só as outras — confirmado ao vivo (pedido seguinte com o mesmo cookie devolveu 401); (5) os 6 endpoints de gestão de 2FA (`status/email-enviar/setup/enable/email-reenviar/disable`) são distintos do desafio de 2FA no momento de login já confirmado numa sessão anterior; testados ao vivo o fluxo TOTP completo (setup → código calculado localmente com o segredo devolvido → enable → confirmação `metodo:"TOTP"` → disable com o código certo, e 401 com um código errado) e o estado `emailIndisponivel` (mensagem exacta confirmada, `EMAIL_PROVIDER=console` neste ambiente); (6) `POST /api/users/me/anonimizar` valida a senha com **422**, não 401 (diferente de `PATCH /password`) — confirmado ao vivo; o sucesso NÃO foi testado ao vivo por ser irreversível (anonimiza e fecha a conta) — só o caminho de erro (senha errada); (7) `PATCH /api/notifications/:id/read` não verifica posse (comportamento herdado do Node, documentado como deliberado no próprio Java) e não há caso especial para "já lida" — confirmado ao vivo, reproduzido tal e qual, sem tentar "corrigir" no Angular. `resolverDestinoNotificacao()` portado 1:1 para `shared/domain.ts`. Testado ao vivo por HTTP directo com a conta Comprador de teste (perfil, avatar upload/remove com PNG real, 2FA TOTP completo, GDPR export com as 5 categorias confirmadas, notificações) e por browser headless com Playwright em duas personas (Comprador e Admin Sistema, confirmando o ramo `company: null` → "Administração KIXIMA"), sem nenhum erro JS de página. Dependência `qrcode` (já usada em `frontend/`) adicionada a `frontend-angular/package.json` para gerar o QR code do 2FA por app.

**Verificação ao vivo — Documentos imprimíveis (FeeStatement/PrintableDocument, partilhadas)**: ambas reutilizam contratos já confirmados (`PlatformFeesService.forCompany()`/`OrdersService.get()`), mas exigiram um agente de pesquisa dedicado para os campos ADICIONAIS que estas duas páginas lêem e que as páginas anteriores a usar os mesmos serviços não precisavam. Confirmados e acrescentados aos modelos: em `CompanyRef` (buyerCompany/supplierCompany de `GET /api/purchase-orders/:id`), 8 campos novos — `taxId, address, province, contactEmail, contactPhone, bankName, iban, swift` — todos sempre presentes (nunca omitidos individualmente); em `PurchaseOrderItemDto.product`, `category`/`kind` (o Java devolve o `ProductDto` completo neste endpoint, não só `{id,name}`); em `InvoiceLineDto`, `quantity/unitPrice/netAmount/ivaAmount`; em `InvoiceDto`, `hashDocumento` e — descoberto por leitura directa do Java durante a implementação, fora do âmbito do agente — `netAmount/taxAmount/withholdingAmount` (a mesma tríade que já existia em `PurchaseOrderDto`, também presente na Fatura). Não há campo pré-formatado para o número de documento AGT (`série.ano/sequencial`) — confirmado que o cliente tem mesmo de o montar, tal como o React. Durante a verificação ao vivo foi encontrada e corrigida uma segunda lacuna real, fora do que o agente cobriu: `PlatformFeeStatementDto.company` tem também `address/city/province/country/contactEmail` (não só `name/taxId/plan`, suficiente para a página anterior a usar este DTO), e `formula.{perPo,perInvoice,thresholdUsd,percentAbove}` são **números**, não strings (ao contrário da maioria dos valores monetários deste contrato) — confirmado ao vivo com `GET /api/companies/:id/platform-fees` real. Testado ao vivo por HTTP directo (PO paga com fatura completa — linhas, pagamento, dados bancários do fornecedor — e uma PO sem fatura) e por browser headless com Playwright em `/documento/po/:id`, `/documento/fatura/:id` (incluindo o estado "ainda não tem fatura emitida" para uma PO rejeitada) e `/documento/taxas/:companyId`, sem nenhum erro JS de página. `formatMoneyDoc()`/`formatDateDoc()`/`formatDateTimeDoc()` extraídos para `shared/print-document.ts` — formato oficial do documento (separador de milhar por ponto, "AOA" como sufixo cru), deliberadamente distinto de `formatMoney()` em `domain.ts` (que usa "Kz").
...
| `Profile.jsx` | 2 | **Migrado** |
| `Security.jsx` | 2 | **Migrado** |
| `Notifications.jsx` | 2 | **Migrado** |
...
| `FeeStatement.jsx` | 2 | **Migrado** |
| `PrintableDocument.jsx` | 2 | **Migrado** |
...
| `Home.jsx` | 2 | **Migrado** |
| `Services.jsx` | 2 | **Migrado** |
| `Explore.jsx` | 2 | **Migrado** |
| `ServiceDetail.jsx` | 2 | **Migrado** |
| `SupplierCompare.jsx` | 2 | **Migrado** |
| `Suppliers.jsx` | 2 | **Migrado** |
...
| `Companies.jsx` | 2 | **Migrado** |
| `Contracts.jsx` | 2 | **Migrado** |
| `Cobrancas.jsx` | 2 | **Migrado** |
| `Home.jsx` | 2 | **Migrado** |
...
1. Escolher o próximo domínio pela tabela de prioridade acima. **Toda a
   ...
   (`Kits,Documents,ProductRanking,Reports,ApiCatalogo`). **O Comprador está
   agora também com toda a prioridade 1 e 2 concluída** — só resta prioridade
   3 (`Activities,CategoryManagement,PoRobot`). `companyAdmin/Assinatura.jsx`
   e `adminSistema/{Companies,Contracts,Cobrancas}.jsx` também estão
   **migradas** — restam do Company Admin e do Admin do Sistema só as páginas
   restantes da tabela acima (ver estado de cada uma, maioritariamente
   prioridade 3). `Profile.jsx`/`Security.jsx`/`Notifications.jsx`/
   `FeeStatement.jsx`/`PrintableDocument.jsx` partilhadas (prioridade 2,
   usadas por todas as personas) também estão **migradas**, incluindo as
   rotas globais `/perfil`, `/seguranca`, `/notificacoes` e `/documento/*`.
   Só restam de prioridade 2 partilhada `Suporte`/`ChatComercial` (dependem
   de tempo real — ver secção "Tempo real", ainda não portada) e
   `SuporteFeedback`/`SupplierDevelopment`/`Plans` (prioridade 3). Os
   próximos candidatos com bom custo/benefício: começar a prioridade 3 do
   Fornecedor/Comprador/Admin do Sistema (nenhuma dependência de tempo real),
   ou portar o adaptador STOMP para desbloquear Chat/Suporte. Repare que
