...
1. Escolher o próximo domínio pela tabela de prioridade acima. **Toda a
   ...
   **migradas** — restam do Company Admin e do Admin do Sistema só as páginas
   ...
   partilhadas (prioridade 2, usadas por todas as personas) também estão
   **migradas**, incluindo as rotas globais `/perfil`, `/seguranca` e
   `/notificacoes`. Os próximos candidatos com bom custo/benefício:
   `Suporte`/`ChatComercial`/`FeeStatement`/`PrintableDocument` (prioridade 2,
   partilhadas, envolvem tempo real — ver secção "Tempo real"), ou começar a
   prioridade 3 do Fornecedor/Comprador/Admin do Sistema. Repare que
