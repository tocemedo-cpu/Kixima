...
router.post('/extrato', requireRole('FINANCEIRO', 'ADMIN_SISTEMA'), async (req, res) => {
...
router.get('/por-resolver', requireRole('FINANCEIRO', 'ADMIN_SISTEMA'), async (req, res) => {
  res.json(await conciliacao.porResolver(req.query));
});

router.post('/:id/conciliar', requireRole('FINANCEIRO', 'ADMIN_SISTEMA'), async (req, res) => {
...
router.get('/canais', requireRole('FINANCEIRO', 'ADMIN_SISTEMA'), (req, res) => {
  res.json({
    canais: [
