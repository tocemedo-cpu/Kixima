...
// quando a AGT NÃO está configurada neste ambiente (o estado real hoje — ver
// agtSigningService.js/agtSandboxClient.js). Antes disto, o pedido rebentava
// dentro do serviço com um Error genérico e o errorHandler devolvia um 500
// sem contexto nenhum; agora cada rota verifica a configuração ANTES de
// tentar assinar/submeter e devolve 503 com uma mensagem clara.
// /agt-payload só assina (exigirAssinaturaAgtConfigurada); /agt-serie-payload
// agora SUBMETE à AGT também, por isso exige a Sandbox inteira
// (exigirSandboxAgtConfigurada, superset da assinatura).
//
...
describe('GET /api/faturacao/agt-serie-payload sem a Sandbox da AGT configurada', () => {
  // Esta rota SUBMETE à AGT (agtSeriesService.solicitarSerie(), via
  // agtSandboxClient.js) — por isso exige a configuração da Sandbox
  // (superset da assinatura: chave + validação + utilizador/senha), não só
  // a assinatura, ver exigirSandboxAgtConfigurada() em faturacaoRoutes.js.
  test('devolve 503 com mensagem clara, não um 500 genérico', async () => {
    const res = await auth(tokens.adminSistema).get('/api/faturacao/agt-serie-payload').query({ ano: '2026', tipoDocumento: 'FT' });
    expect(res.status).toBe(503);
    expect(res.body.error.code).toBe('SERVICO_INDISPONIVEL');
    expect(res.body.error.message).toMatch(/ligação à Sandbox da AGT ainda não está configurada/);
  });
